...
def _cluster_stat(stat, clusters, statistic='cluster_mass'):
    ...
    if (statistic == 'cluster_mass'):
        ...
        if (n_clusters == 0):
            ...
            cstat = np.array([np.max(np.abs(stat))])
            return cstat
    else:
...
def permutation_test(X, Y, n_permutations=1000, threshold=1, tail='both',
                     alpha=0.05, equal_var=False, t_vals=None, cluster_stat="cluster_mass", abs=True):
    ...
    cluster_stat   - Multiple comparison correction statistic ("cluster_mass" or "tmax")
    ...
    if t_vals is not None:
        ...
        ref_tstat, _ = ttest_ind(X, Y, equal_var=equal_var)
    if abs:
        ref_tstat = np.abs(ref_tstat)
    ref_clusters = _cluster_by_adjacency(ref_tstat > threshold)
    ref_cstat = _cluster_stat(ref_tstat, ref_clusters, cluster_stat)
    ...
    t_perms = np.zeros((n_permutations, Z.shape[1]))

    for p in arange(0, n_permutations):
        """Do a random partition of the data."""
        # print("Permutation {} of {}".format(p, n_permutations))
        ...
        tstat, _ = ttest_ind(T, U, equal_var=equal_var)
        t_perms[p, :] = tstat

        if abs:
            tstat = np.abs(tstat)
        if t_vals is not None:
            tstat = cluster.tfce(tstat, dt=.1, E=0.5, H=2.0, connectivity=None)
            # tstat = np.abs(tstat)
        clusters = _cluster_by_adjacency(tstat > threshold)

        """Compute cluster-level statistics."""
        cstat[p] = np.max(_cluster_stat(tstat, clusters, cluster_stat))

    """Compute p-values for each cluster. Make a vector indicating which
    variables are significant."""
    if cluster_stat == "tmax":
        pvals = np.array([
            (100 - stats.percentileofscore(cstat, t, kind='strict')) / 100
            for t in ref_tstat])
    else:
        pvals = [np.sum(cstat > c) / float(n_permutations) for c in ref_cstat]

    # Uncorrected
    p_uncorrected = np.sum(t_perms >= ref_tstat, 0) / float(n_permutations)
    ...
    return ref_tstat, p_uncorrected, significant, pvals, cstat, ref_cstat, ref_clusters
